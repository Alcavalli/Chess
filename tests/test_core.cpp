...
{
    if (!sq) return '.';
    if (sq->type == PieceType::Pawn)    return (sq->color == PieceColor::White) ? 'P' : 'p';
    if (sq->type == PieceType::Rook)    return (sq->color == PieceColor::White) ? 'R' : 'r';
    if (sq->type == PieceType::Knight)    return (sq->color == PieceColor::White) ? 'N' : 'n';
    if (sq->type == PieceType::Bishop)    return (sq->color == PieceColor::White) ? 'B' : 'b';
    if (sq->type == PieceType::Queen)    return (sq->color == PieceColor::White) ? 'Q' : 'q';
    if (sq->type == PieceType::King)    return (sq->color == PieceColor::White) ? 'K' : 'k';
    return '#';
}
...
int main()
{
    ...
    std::cout << std::boolalpha << "Check: " << MoveGenerator::isInCheck(board, PieceColor::White) << '\n';
